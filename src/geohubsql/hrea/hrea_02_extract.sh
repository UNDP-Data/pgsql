...
cat "$hrea_dir"'extents.txt'|sed 's/GID_0_//g'|grep Z|awk \
  ...
  -v adm0_dir="$adm0_dir" \
  '{print "exactextract " \
"-r @pop:@"fb_pop_dir""$1"_pop.tif@ " \
"-r @"this_series"_2012:@"thr_dir"/2012/"this_series"_2012_"$1"_m80.tif@ " \
"-p @"adm0_dir"GID_0_"$1".gpkg@ -f @GID_2b@ -o @"this_series"_"$1".csv@ --progress -s @sum(pop)@ -s @mean(pop)@ -s @min(pop)@ -s @max(pop)@ " \
"-s @popw_2012=weighted_mean("this_series"_2013,pop)@" }' |tr '@' '"'|parallel -I{} echo {}
