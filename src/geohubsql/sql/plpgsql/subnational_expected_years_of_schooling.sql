CREATE OR REPLACE FUNCTION admin.subnational_expected_years_of_schooling(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'admin.subnational_expected_years_of_schooling';
...
--        RAISE WARNING 'Uding implified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.gdlcode,
			CAST(h.expected_years_of_schooling as FLOAT)
            FROM admin."%s" a
...
COMMENT ON FUNCTION admin.subnational_expected_years_of_schooling IS 'This is the subnational HDI component expected_years_of_schooling';


--
--SELECT * FROM admin.subnational_expected_years_of_schooling(0,0,0) AS OUTP;

-- example URL:
-- wget http://172.18.0.6:7800/admin.subnational_expected_years_of_schooling/0/0/0.pbf
-- http://172.18.0.6:7800/admin.subnational_expected_years_of_schooling/{z}/{x}/{y}.pbf
-- works in QGIS:
-- http://172.18.0.6:7800/admin.subnational_expected_years_of_schooling/{z}/{x}/{y}.pbf
