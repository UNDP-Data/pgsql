CREATE OR REPLACE FUNCTION admin.subnational_gross_national_income(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'admin.subnational_gross_national_income';
...
--        RAISE WARNING 'Uding implified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.gdlcode,
			CAST(h.gross_national_income as FLOAT)
            FROM admin."%s" a
...
COMMENT ON FUNCTION admin.subnational_expected_years_of_schooling IS 'This is the subnational HDI component gross_national_income';


--
--SELECT * FROM admin.subnational_gross_national_income(0,0,0) AS OUTP;

-- example URL:
-- wget http://172.18.0.6:7800/admin.subnational_gross_national_income/0/0/0.pbf
-- http://172.18.0.6:7800/admin.subnational_gross_national_income/{z}/{x}/{y}.pbf
-- works in QGIS:
-- http://172.18.0.6:7800/admin.subnational_gross_national_income/{z}/{x}/{y}.pbf
