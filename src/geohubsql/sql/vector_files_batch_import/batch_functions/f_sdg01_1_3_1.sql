CREATE OR REPLACE FUNCTION sdg01.f_1_3_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg01.f_1_3_1';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2020 , a.value_2017 , a.value_2019 , a.value_2018 , a.value_2016 
             , a.value_latest 
			FROM sdg01.admin0 a
			WHERE
    			indicator = ''1.3.1''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2020 as FLOAT) , CAST(h.value_2017 as FLOAT) , CAST(h.value_2019 as FLOAT) , CAST(h.value_2018 as FLOAT) , CAST(h.value_2016 as FLOAT) 
...
COMMENT ON FUNCTION sdg01.f_1_3_1 IS 'This is f_1_3_1
';

--SELECT * FROM "sdg01"."f_1_3_1"(0,0,0,'{"subsets":
--    {"sex_code": {"options": ["M", "_T", "F"], "value": "M"}}
--    }') AS OUTP;