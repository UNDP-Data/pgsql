CREATE OR REPLACE FUNCTION sdg03.f_3_c_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"sex_code": {"options": ["M", "F"], "value": "M"}, "type_of_oc": {"options": ["ISCO08_2221_3221", "ISCO08_221"], "value": "ISCO08_2221_3221"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg03.f_3_c_1';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
			...
			WHERE
    			indicator = ''3.c.1''
			AND a.sex_code = ''%s'' AND a.type_of_oc = ''%s'' 
...
COMMENT ON FUNCTION sdg03.f_3_c_1 IS 'This is f_3_c_1
';

--SELECT * FROM "sdg03"."f_3_c_1"(0,0,0,'{"subsets":
--    {"sex_code": {"options": ["M", "F"], "value": "M"}, "type_of_oc": {"options": ["ISCO08_2221_3221", "ISCO08_221"], "value": "ISCO08_2221_3221"}}
--    }') AS OUTP;