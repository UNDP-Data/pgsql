    ...
    "subsets":
    {"age_code": {"options": ["Y16T65"], "value": "Y16T65"}, "sex_code": {"options": ["M", "F", "_T"], "value": "M"}, "type_of_skill": {"options": ["SKILL_LTRCY", "SKILL_NMRCY"], "value": "SKILL_LTRCY"}}
    }'
    )
--    "subsets":
--    {
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
			...
			WHERE
    			indicator = ''4.6.1''
			AND a.age_code = ''%s'' AND a.sex_code = ''%s'' AND a.type_of_skill = ''%s'' 
             AND a.value_latest IS NOT NULL 
        );'
        , age_code , sex_code , type_of_skill 
        ) INTO my_query;
...
COMMENT ON FUNCTION sdg04.f_4_6_1 IS 'This is f_4_6_1
';
