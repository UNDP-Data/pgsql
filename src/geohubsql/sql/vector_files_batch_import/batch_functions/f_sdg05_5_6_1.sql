CREATE OR REPLACE FUNCTION sdg05.f_5_6_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg05.f_5_6_1';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2006 , a.value_2015 , a.value_2011 
             , a.value_latest 
			FROM sdg05.admin0 a
			WHERE
    			indicator = ''5.6.1''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2006 as FLOAT) , CAST(h.value_2015 as FLOAT) , CAST(h.value_2011 as FLOAT) 
...
COMMENT ON FUNCTION sdg05.f_5_6_1 IS 'This is f_5_6_1
';

--SELECT * FROM "sdg05"."f_5_6_1"(0,0,0,'{"subsets":
--    {"age_code": {"options": ["Y15T49"], "value": "Y15T49"}, "sex_code": {"options": ["F"], "value": "F"}}
--    }') AS OUTP;