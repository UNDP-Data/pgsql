CREATE OR REPLACE FUNCTION sdg06.f_6_5_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"location": {"options": ["ALLAREA"], "value": "ALLAREA"}, "education": {"options": ["_T"], "value": "_T"}, "type_of_pr": {"options": ["ALP"], "value": "ALP"}, "type_of_oc": {"options": ["isco08"], "value": "isco08"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg06.f_6_5_1';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2020 , a.value_2017 
             , a.value_latest 
			FROM sdg06.admin0 a
			WHERE
    			indicator = ''6.5.1''
			AND a.location = ''%s'' AND a.education = ''%s'' AND a.type_of_pr = ''%s'' AND a.type_of_oc = ''%s'' 
             AND a.value_latest IS NOT NULL 
        );'
        , location , education , type_of_pr , type_of_oc 
        ) INTO my_query;
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2020 as FLOAT) , CAST(h.value_2017 as FLOAT) 
...
COMMENT ON FUNCTION sdg06.f_6_5_1 IS 'This is f_6_5_1
';

--SELECT * FROM "sdg06"."f_6_5_1"(0,0,0,'{"subsets":
--    {"location": {"options": ["ALLAREA"], "value": "ALLAREA"}, "education": {"options": ["_T"], "value": "_T"}, "type_of_pr": {"options": ["ALP"], "value": "ALP"}, "type_of_oc": {"options": ["isco08"], "value": "isco08"}}
--    }') AS OUTP;