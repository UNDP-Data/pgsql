CREATE OR REPLACE FUNCTION sdg06.f_6_b_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"location": {"options": ["R"], "value": "R"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg06.f_6_b_1';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2017 , a.value_2012 , a.value_2010 , a.value_2019 , a.value_2014 
             , a.value_latest 
			FROM sdg06.admin0 a
			WHERE
    			indicator = ''6.b.1''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2017 as FLOAT) , CAST(h.value_2012 as FLOAT) , CAST(h.value_2010 as FLOAT) , CAST(h.value_2019 as FLOAT) , CAST(h.value_2014 as FLOAT) 
...
COMMENT ON FUNCTION sdg06.f_6_b_1 IS 'This is f_6_b_1
';

--SELECT * FROM "sdg06"."f_6_b_1"(0,0,0,'{"subsets":
--    {"location": {"options": ["R"], "value": "R"}}
--    }') AS OUTP;