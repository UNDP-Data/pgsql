CREATE OR REPLACE FUNCTION sdg07.f_7_1_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"location": {"options": ["R"], "value": "R"}, "reportin_1": {"options": ["Global"], "value": "Global"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg07.f_7_1_1';
...
-- PL/PgSQL function to create a dynamic function layer (delivered as Vector Tiles) with filters
    ...
    BEGIN
        ...
        reportin_1   := requested_json->'subsets'->'reportin_1'->>'value';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
			...
			WHERE
    			indicator = ''7.1.1''
			AND a.location = ''%s'' AND a.reportin_1 = ''%s'' 
...
COMMENT ON FUNCTION sdg07.f_7_1_1 IS 'This is f_7_1_1
';

--SELECT * FROM "sdg07"."f_7_1_1"(0,0,0,'{"subsets":
--    {"location": {"options": ["R"], "value": "R"}, "reportin_1": {"options": ["Global"], "value": "Global"}}
--    }') AS OUTP;