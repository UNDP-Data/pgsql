CREATE OR REPLACE FUNCTION sdg08.f_8_3_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"sex_code": {"options": ["M", "_O", "F", "_T"], "value": "M"}, "activity_c": {"options": ["ISIC4_A", "ISIC4_BTU", "_T"], "value": "ISIC4_A"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg08.f_8_3_1';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2014 , a.value_2011 , a.value_2019 
             , a.value_latest 
			FROM sdg08.admin0 a
			WHERE
    			indicator = ''8.3.1''
			AND a.sex_code = ''%s'' AND a.activity_c = ''%s'' 
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2014 as FLOAT) , CAST(h.value_2011 as FLOAT) , CAST(h.value_2019 as FLOAT) 
...
COMMENT ON FUNCTION sdg08.f_8_3_1 IS 'This is f_8_3_1
';

--SELECT * FROM "sdg08"."f_8_3_1"(0,0,0,'{"subsets":
--    {"sex_code": {"options": ["M", "_O", "F", "_T"], "value": "M"}, "activity_c": {"options": ["ISIC4_A", "ISIC4_BTU", "_T"], "value": "ISIC4_A"}}
--    }') AS OUTP;