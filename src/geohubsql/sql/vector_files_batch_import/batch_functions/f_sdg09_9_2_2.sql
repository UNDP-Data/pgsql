CREATE OR REPLACE FUNCTION sdg09.f_9_2_2(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg09.f_9_2_2';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2014 , a.value_2011 , a.value_2004 , a.value_2019 
             , a.value_latest 
			FROM sdg09.admin0 a
			WHERE
    			indicator = ''9.2.2''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2014 as FLOAT) , CAST(h.value_2011 as FLOAT) , CAST(h.value_2004 as FLOAT) , CAST(h.value_2019 as FLOAT) 
...
COMMENT ON FUNCTION sdg09.f_9_2_2 IS 'This is f_9_2_2
';

--SELECT * FROM "sdg09"."f_9_2_2"(0,0,0,'{"subsets":
--    {"activity_c": {"options": ["ISIC4_C"], "value": "ISIC4_C"}}
--    }') AS OUTP;