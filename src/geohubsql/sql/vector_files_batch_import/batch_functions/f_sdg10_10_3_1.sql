CREATE OR REPLACE FUNCTION sdg10.f_10_3_1(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"sex_code": {"options": ["M", "F", "_T"], "value": "M"}, "disability": {"options": ["Persons with disability", "Persons without disability", "No breakdown by disability"], "value": "Persons with disability"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg10.f_10_3_1';
...
-- PL/PgSQL function to create a dynamic function layer (delivered as Vector Tiles) with filters
    ...
    BEGIN
        ...
        disability   := requested_json->'subsets'->'disability'->>'value';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
			...
			WHERE
    			indicator = ''10.3.1''
			AND a.sex_code = ''%s'' AND a.disability = ''%s'' 
...
COMMENT ON FUNCTION sdg10.f_10_3_1 IS 'This is f_10_3_1
';

--SELECT * FROM "sdg10"."f_10_3_1"(0,0,0,'{"subsets":
--    {"sex_code": {"options": ["M", "F", "_T"], "value": "M"}, "disability": {"options": ["Persons with disability", "Persons without disability", "No breakdown by disability"], "value": "Persons with disability"}}
--    }') AS OUTP;