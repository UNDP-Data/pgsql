CREATE OR REPLACE FUNCTION sdg10.f_10_7_2(
    z integer default 0,
    x integer default 0,
    y integer default 0,
    params varchar default '{
    "subsets":
    {"policy_d_1": {"options": ["Domain 6. Safe, orderly and regular migration", "Domain 3. Cooperation and partnerships", "Domain 5. Mobility dimensions of crises", "All Domains", "Domain 2. Whole-of-government/ Evidence-based policies", "Domain 4. Socioeconomic well-being", "Domain 1. Migrant rights"], "value": "Domain 6. Safe, orderly and regular migration"}}
    }'
    )
--    "subsets":
--    {
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg10.f_10_7_2';
...
-- PL/PgSQL function to create a dynamic function layer (delivered as Vector Tiles) with filters
    ...
    BEGIN
        ...
        policy_d_1   := requested_json->'subsets'->'policy_d_1'->>'value';
...
COMMENT ON FUNCTION sdg10.f_10_7_2 IS 'This is f_10_7_2
';

--SELECT * FROM "sdg10"."f_10_7_2"(0,0,0,'{"subsets":
--    {"policy_d_1": {"options": ["Domain 6. Safe, orderly and regular migration", "Domain 3. Cooperation and partnerships", "Domain 5. Mobility dimensions of crises", "All Domains", "Domain 2. Whole-of-government/ Evidence-based policies", "Domain 4. Socioeconomic well-being", "Domain 1. Migrant rights"], "value": "Domain 6. Safe, orderly and regular migration"}}
--    }') AS OUTP;