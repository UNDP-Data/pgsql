CREATE OR REPLACE FUNCTION sdg11.f_11_6_2(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg11.f_11_6_2';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2013 , a.value_2012 , a.value_2011 , a.value_2014 , a.value_2015 , a.value_2016 
             , a.value_latest 
			FROM sdg11.admin0 a
			WHERE
    			indicator = ''11.6.2''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2013 as FLOAT) , CAST(h.value_2012 as FLOAT) , CAST(h.value_2011 as FLOAT) , CAST(h.value_2014 as FLOAT) , CAST(h.value_2015 as FLOAT) , CAST(h.value_2016 as FLOAT) 
...
COMMENT ON FUNCTION sdg11.f_11_6_2 IS 'This is f_11_6_2
';

--SELECT * FROM "sdg11"."f_11_6_2"(0,0,0,'{"subsets":
--    {"location": {"options": ["R", "_T", "U"], "value": "R"}}
--    }') AS OUTP;