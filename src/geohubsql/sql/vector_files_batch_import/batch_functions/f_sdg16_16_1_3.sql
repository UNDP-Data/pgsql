CREATE OR REPLACE FUNCTION sdg16.f_16_1_3(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg16.f_16_1_3';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2013 , a.value_2012 , a.value_2011 , a.value_2010 , a.value_2009 , a.value_2008 
             , a.value_latest 
			FROM sdg16.admin0 a
			WHERE
    			indicator = ''16.1.3''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2013 as FLOAT) , CAST(h.value_2012 as FLOAT) , CAST(h.value_2011 as FLOAT) , CAST(h.value_2010 as FLOAT) , CAST(h.value_2009 as FLOAT) , CAST(h.value_2008 as FLOAT) 
...
COMMENT ON FUNCTION sdg16.f_16_1_3 IS 'This is f_16_1_3
';

--SELECT * FROM "sdg16"."f_16_1_3"(0,0,0,'{"subsets":
--    {"sex_code": {"options": ["M", "_T", "F"], "value": "M"}}
--    }') AS OUTP;