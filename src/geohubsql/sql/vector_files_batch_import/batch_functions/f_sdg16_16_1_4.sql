CREATE OR REPLACE FUNCTION sdg16.f_16_1_4(
    z integer default 0,
    x integer default 0,
    y integer default 0,
...
RETURNS bytea AS $$
    ...
    DECLARE
        mvt bytea;
        layer_name varchar := 'sdg16.f_16_1_4';
...
--        RAISE WARNING 'JSON: %',my_query;
        ...
        SELECT format('CREATE TEMPORARY TABLE sdg_tmp_table AS (
            SELECT
			a."iso3cd" AS iso3cd
			, a.value_2013 , a.value_2017 , a.value_2014 , a.value_2015 , a.value_2016 
             , a.value_latest 
			FROM sdg16.admin0 a
			WHERE
    			indicator = ''16.1.4''
...
--        RAISE WARNING 'Using simplified table %', simplified_table_name;
        ...
        EXECUTE format('CREATE TEMPORARY TABLE mvtgeom AS (
			...
			a.iso3cd
            , CAST(h.value_2013 as FLOAT) , CAST(h.value_2017 as FLOAT) , CAST(h.value_2014 as FLOAT) , CAST(h.value_2015 as FLOAT) , CAST(h.value_2016 as FLOAT) 
...
COMMENT ON FUNCTION sdg16.f_16_1_4 IS 'This is f_16_1_4
';

--SELECT * FROM "sdg16"."f_16_1_4"(0,0,0,'{"subsets":
--    {"sex_code": {"options": ["M", "_T", "F"], "value": "M"}}
--    }') AS OUTP;